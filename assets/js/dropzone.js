...
$(document).ready(function () {
    ...
    // <?php echo $user_img_profile; ?>
    var $img_dir_name2 = '';
    Dropzone.options.dropzonewidgetuser = {
        paramName: "file",
        maxFilesize: 1,
        maxFiles: 1,
        ...
        init: function () {
            ...
            this.on("success", function (file, response) {
                $img_dir_name2 = data.FILE_URI + response;
            })
        }
    }
    $('#dropzonewidgetuser').dropzone();
    //save imgae
    $(".save-profile-image").click(function (e) {
        ...
        $.ajax({
            ...
            type: "post",
            data: {
                action: "user_profile_image_action",
                user_profile_image: $img_dir_name2,
            },
            success: function (response) {
                if (response == 1) {
                    $('.response-box-c').addClass('success-box');
                    $('.response-box-c span').text('تصویر پروفایل شما با موفقیت آپلود شد');
                    $('.response-box').fadeIn(200)
                } else if (response == 0) {
                    $('.response-box-c').addClass('error-box');
                    $('.response-box-c span').text('خطایی در آپلود تصویر به وجود امد');
                    $('.response-box').fadeIn(200)
                }else if (response == -1) {
                    $('.response-box-c').addClass('error-box');
                    $('.response-box-c span').text('ابتدا تصویر خود را آپلود نمایید');
                    $('.response-box').fadeIn(200)
                }
            },
        });
